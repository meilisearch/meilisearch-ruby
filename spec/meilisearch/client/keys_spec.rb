...
RSpec.describe 'MeiliSearch::Client - Keys' do
  ...
  it 'fails to get settings if public key used' do
    ...
    expect { new_client.index(@uid).settings }.to raise_meilisearch_api_error_with(403, 'invalid_token', 'authentication_error')
  end
  ...
  it 'fails to get keys if private key used' do
    ...
    expect { new_client.keys }.to raise_meilisearch_api_error_with(403, 'invalid_token', 'authentication_error')
  end

  it 'fails to search if no key used' do
    new_client = MeiliSearch::Client.new($URL)
    expect { new_client.index(@uid).settings }.to raise_meilisearch_api_error_with(401, 'missing_authorization_header', 'authentication_error')
  end
